...
VALID = {
    "primary": {"font": "satoshi", "weight": 400},
    "label": {"font": "ibm-plex-mono", "weight": 700},
...
def test_a_second_put_replaces_rather_than_duplicating(client):
    ...
    changed = {**VALID, "mono": {"font": "jetbrains-mono", "weight": 500}}
    client.put("/api/appearance", json=changed)
    assert client.get("/api/appearance").json()["appearance"] == changed


@pytest.mark.parametrize("bad, why", [
    ({**VALID, "mono": {"font": "x", "weight": 950}}, "weight above the CSS range"),
    ({**VALID, "mono": {"font": "x", "weight": 50}}, "weight below the CSS range"),
    ({**VALID, "mono": {"font": "", "weight": 400}}, "empty font id"),
